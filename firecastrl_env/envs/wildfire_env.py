...
class WildfireEnv(gym.Env):
    ...
    def __init__(self, env_id: int = 0, num_agents: int = 3, render_mode: Optional[str] = None):
        super().__init__()

        # Track number of helicopters
        ...
        self.observation_space = spaces.Dict({
            ...
            'helicopter_coord': spaces.Box(
                low=0, 
                high=max(config.gridWidth, config.gridHeight), 
    ...
    def _reset_state_variables(self) -> None:
        ...
        # Initialize N helicopters at different starting positions we are staggering them slightly so they don't stack
        start_x = 70
        start_y = 30
        initial_coords = np.array([
            [start_x + (i * 5), start_y] for i in range(self.num_agents)
        ], dtype=np.int32)

        self.state = {
            'cells': np.zeros((160, 240), dtype=np.float64),
            'helicopter_coord': initial_coords, # Shape (N, 2)
            'quenched_cells': np.array([0], dtype=np.float32),
            'last_action': [None] * self.num_agents # Track history per agent
        }
    ...
    def step(self, action):
        # action is now a numpy array or list, e.g., [0, 4, 2]
        total_quenched = 0
        
        # Loop through each agent
        for i in range(self.num_agents):
            agent_action = action[i]
            self.state['last_action'][i] = agent_action # Store for reward calculation
            
            # Extract current coords for this agent
            hx, hy = self.state['helicopter_coord'][i]
            
            # Movement Logic
            if agent_action == 0:   # Down
                hy += config.HELICOPTER_SPEED
            elif agent_action == 1: # Up
                hy -= config.HELICOPTER_SPEED
            elif agent_action == 2: # Left
                hx -= config.HELICOPTER_SPEED
            elif agent_action == 3: # Right
                hx += config.HELICOPTER_SPEED
            elif agent_action == 4: # Helitack
                # Perform drop
                quenched = helper.perform_helitack(self.cells, hx, hy)
                total_quenched += quenched
                
                # Add to visualization history
                self.helitack_history.append((hx, hy, self.step_count))
            
            # Clip coordinates
            hx = int(np.clip(hx, 0, 239))
            hy = int(np.clip(hy, 0, 159))
            
            # Update this specific agent's position in the state
            self.state['helicopter_coord'][i] = [hx, hy]

        # Clean up visualization history
        self.helitack_history = [(x, y, s) for x, y, s in self.helitack_history if self.step_count - s < 20]

        # Run Fire Simulation (Physics runs once per global step, not per agent)
        self.tick(self._step_delta)
        self.step_count += 1
        
        # Update State Stats
        self.state['quenched_cells'] = np.array([total_quenched], dtype=np.float32)
        cells_burning, cells_burnt = self._get_current_fire_stats()

        # CALCULATE REWARD (This needs your new logic)
        reward = self.calculate_multiagent_reward(cells_burning, total_quenched)
        ...
        observation = {
            'cells': helper.ignition_times(self.cells, self.gridWidth, self.gridHeight),
            'helicopter_coord': self.state['helicopter_coord'].copy(),
            'quenched_cells': self.state['quenched_cells']
        }
    ...
    def calculate_multiagent_reward(self, curr_burning: int, total_extinguished: int) -> float:
        reward = 0.0
        
        # 1. Global Reward (Cooperative Goal)
        reward += 5.0 * total_extinguished
        reward -= 0.05 * curr_burning
        reward -= 0.01 # Time penalty

        # 2. Individual "Wasted Water" Penalty
        # We need to check every agent who attempted to drop water (Action 4)
        cells = np.array(self.state['cells']) # Note: You might need to access raw cells differently depending on helper
        fire_states = cells // 3 # Approximation based on your logic, or access .fireState directly from object list
        
        for i in range(self.num_agents):
            last_act = self.state['last_action'][i]
            hx, hy = self.state['helicopter_coord'][i]
            
            if last_act == 4:
                # We can't easily check "did THIS agent extinguish?" because `total_extinguished` is aggregated.
                # However, we can check if they dropped on a valid target.
                
                # Logic: If they dropped on a cell that is NOT burning, penalize.
                # You will need to fetch the specific cell object to be accurate.
                cell_idx = helper.get_grid_index_for_location(hx, hy, self.gridWidth)
                cell = self.cells[cell_idx]
                
                # If they dropped and it wasn't burning (Wasted Water)
                # Note: fireState logic might need to be imported from enums
                if cell.fireState != 1: # Assuming 1 is Burning
                    reward -= 2.0 # The "Wasted Water" penalty
    ...
    def _render_human(self):
            ...
            if self._renderer is None:
                ...
                pygame.display.set_caption("Wildfire Environment - FirecastRL (Multi-Agent)")
            ...
            # --- CHANGED: Loop to draw ALL helicopters ---
            for i in range(self.num_agents):
                hx, hy = int(self.state['helicopter_coord'][i][0]), int(self.state['helicopter_coord'][i][1])
                heli_screen_x = hx * 4 + 2
                heli_screen_y = hy * 4 + 2
                
                # Helicopter colors (Make different agents slightly different colors if desired, or all yellow)
            ...
            text = self._renderer['font'].render(
                f"Step: {self.step_count} | Burning: {cells_burning} | Burnt: {cells_burnt} | Agents: {self.num_agents}",
                True,
                (255, 255, 255),
                (0, 0, 0)
            )
